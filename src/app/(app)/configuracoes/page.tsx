...
export default async function ConfiguracoesPage() {
  ...
  const currentResident = snapshot.residents.find((resident) => resident.isCurrentUser);
  const currentAvatar = currentResident?.avatar ?? user.nome.slice(0, 2).toUpperCase();
  const hasAvatarUrl = /^https?:\/\//i.test(currentAvatar);
          ...
          <div className="flex items-start gap-4 sm:gap-5">
            {hasAvatarUrl ? (
              <Image
                ...
                height={80}
                unoptimized
                className="h-16 w-16 shrink-0 rounded-none border-[3px] border-neo-dark object-cover shadow-[4px_4px_0_#0F172A] sm:h-20 sm:w-20 sm:border-4"
              />
            ) : (
              <div className="grid h-16 w-16 shrink-0 place-items-center rounded-none border-[3px] border-neo-dark bg-neo-cyan font-heading text-2xl uppercase text-neo-dark shadow-[4px_4px_0_#0F172A] sm:h-20 sm:w-20 sm:border-4 sm:text-3xl">
                {currentAvatar}
              </div>
            )}
            <div>
