...
import { STATUS_PAID, STATUS_PENDING, dateInputValue, formatDueLabel, getMonthLabel, getMonthYear, getUserWithHouse, mapHouseBill, sumBy, toBillStatus } from "./_shared";
...
export const personalRepository = {
  async createIncome(userId: string, input: CreateIncomeInput) {
    await prisma.transacao.create({
      data: { moradorId: userId, titulo: input.titulo, valorCentavos: input.valorCentavos, categoria: "SALARIO", dataVencimento: input.recebidaEm, escopo: EscopoTransacao.PESSOAL, tipo: TipoTransacao.RECEITA, frequencia: "UNICA", status: StatusTransacao.CONCLUIDA }
    });
  },
  async updateIncome(userId: string, incomeId: string, input: UpdateIncomeInput) {
    ...
    await prisma.transacao.update({ where: { id: incomeId }, data: { titulo: input.titulo, valorCentavos: input.valorCentavos, dataVencimento: input.recebidaEm } });
  },
  ...
  async createPersonalBill(userId: string, input: CreatePersonalBillInput) {
    await prisma.transacao.create({
      data: { moradorId: userId, titulo: input.titulo, categoria: input.categoria, valorCentavos: input.valorCentavos, dataVencimento: input.vencimento, observacao: input.observacao, escopo: EscopoTransacao.PESSOAL, tipo: TipoTransacao.DESPESA, frequencia: "UNICA", status: StatusTransacao.PENDENTE }
    });
  },
  ...
  async getPersonalSnapshot(userId: string): Promise<PersonalSnapshot> {
    ...
    const weeklyBills = personalBills.slice(0, 3).map((item) => mapHouseBill({ ...item, vencimento: item.vencimento, pagaEm: null }, false));
