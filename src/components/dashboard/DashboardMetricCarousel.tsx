...
export function DashboardMetricCarousel({ items }: { items: DashboardMetricItem[] }) {
  ...
  function scrollByDirection(direction: "left" | "right") {
    ...
    const amount = Math.max(container.clientWidth * 0.52, 260);
    container.scrollBy({
      ...
      <div className="mb-3 flex items-center justify-between gap-3 xl:mb-4">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <button
            ...
            className="flex h-12 w-12 items-center justify-center border-4 border-neo-dark bg-white text-neo-dark shadow-[4px_4px_0_#0F172A] transition-all hover:-translate-y-1 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0"
          ...
          <button
            ...
            className="flex h-12 w-12 items-center justify-center border-4 border-neo-dark bg-neo-yellow text-neo-dark shadow-[4px_4px_0_#0F172A] transition-all hover:-translate-y-1 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:translate-y-0"
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="flex h-[calc(100%-56px)] flex-col justify-between space-y-3 p-4 sm:p-5 xl:p-6">
              <h2 className="font-heading text-4xl uppercase leading-none text-neo-dark sm:text-5xl xl:text-[4rem] 2xl:text-[4.4rem]">
                {item.value}
              </h2>
              <p className="max-w-[18rem] font-body text-xs font-bold uppercase tracking-[0.08em] text-neo-dark/65 sm:text-sm sm:tracking-[0.12em] xl:max-w-[20rem] xl:text-base">
                {item.description}
              </p>
            </div>
          </NeoCard>
