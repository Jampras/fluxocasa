      ...
      <div className={twMerge("grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 xl:gap-4", desktopColumnsClass)}>
        {items.map((item, index) => (
          <Link
            ...
            href={`#${item.id}`}
            className={twMerge(
              "neo-pressable inline-flex min-h-[54px] items-center justify-center border-[3px] border-neo-dark bg-neo-cream px-3 py-2.5 text-center font-heading text-[0.92rem] uppercase leading-none text-neo-dark shadow-[4px_4px_0_#0F172A] sm:min-h-[58px] sm:border-4 sm:px-4 sm:text-base xl:min-h-[62px] xl:px-5 xl:text-[1.05rem]",
              index % 3 === 0
                ? "hover:bg-neo-yellow"
                : index % 3 === 1
                  ? "hover:bg-neo-cyan"
