...
import type { HouseAuditEvent, HouseBill } from "@/types";
  ...
  return (
    <div className="grid gap-4 sm:gap-6 xl:grid-cols-[1.05fr_0.95fr]">
      <Card className="bg-white p-4 sm:p-5 md:p-6">
        <div className="space-y-1">
          ...
          <p className="text-sm font-bold uppercase tracking-[0.08em] text-neo-dark/65">
            Ultimos pagamentos confirmados dentro do ciclo atual.
          </p>
        </div>
      ...
      <Card className="bg-white p-4 sm:p-5 md:p-6">
        <div className="space-y-1">
          <p className="font-heading text-[10px] uppercase tracking-[0.18em] text-neo-pink sm:text-sm sm:tracking-[0.26em]">
            Movimentacoes da casa
          </p>
          <h3 className="font-heading text-2xl uppercase text-neo-dark sm:text-3xl">Auditoria recente</h3>
          <p className="text-sm font-bold uppercase tracking-[0.08em] text-neo-dark/65">
            Acoes administrativas e mudancas importantes registradas no ambiente.
          </p>
        </div>
        ...
        <div className="mt-4 grid gap-3 sm:mt-5">
          {auditLog.length === 0 ? (
            <div className="border-[3px] border-neo-dark bg-neo-bg px-4 py-5 text-sm font-bold text-neo-dark/65 sm:border-4">
              Nenhuma movimentacao administrativa registrada ainda.
            </div>
          ) : null}
          ...
          {auditLog.map((event) => (
            <div
              key={event.id}
              className="border-[3px] border-neo-dark bg-neo-bg px-4 py-4 shadow-[4px_4px_0_#0F172A] sm:border-4"
            >
              <p className="font-heading text-xl uppercase text-neo-dark sm:text-2xl">{event.title}</p>
              <p className="mt-2 text-sm font-bold text-neo-dark/75">{event.description}</p>
              <p className="mt-3 text-[10px] font-black uppercase tracking-[0.14em] text-neo-pink sm:text-xs">
